...
function Footer() {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      setSending(true)
      await createBoletin(email)
      setMsg({ type: 'ok', text: '¡Suscripción registrada!' })
      setEmail('')
    } catch (err) {
      console.error(err)
      setMsg({ type: 'error', text: 'No se pudo registrar. Intenta de nuevo.' })
    } finally {
      setSending(false)
    }
  }
        ...
        <div className="container">
          <div className="row gy-4">
            ...
            <div className="col-lg-4">
              ...
              <p className="mb-3">Ingresa tu correo electrónico abajo para ser el primero en enterarte de todas las novedades, ofertas y noticias.</p>
              <form className="subscribe-box d-flex" onSubmit={handleSubscribe}>
                ...
                <button className="btn" disabled={sending}>
                  {sending ? 'Enviando...' : 'Suscribirse'}
                </button>
              </form>
              {msg && (
                <small className={msg.type === 'ok' ? 'text-success' : 'text-danger'}>
                  {msg.text}
                </small>
              )}
            </div>
