...
function ShopFrutas() {
  ...
  const getSecondImage = (p) =>
    p?.secondImage || p?.secondimage || p?.image2 || p?.image;
  ...
  const displayedProducts = useMemo(() => {
    ...
    if (filterSortOption === 'new' || filterSortOption === 'sale' ||
        filterSortOption === 'New' || filterSortOption === 'Sale') {
      filtered = filtered.filter(
        (product) => (product.tag || '').toString().toLowerCase() === filterSortOption.toLowerCase()
      );
    }
    ...
    if (filterSortOption === 'high') {
      filtered.sort((a, b) => getPriceNum(b) - getPriceNum(a));
    }
